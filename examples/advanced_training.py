...
def run(version="ml-latest-small", samples=4, batch_size=256, epochs=1):
    ...
    datasets.movielens.download(version)
    ...
    user_x, item_x, y = train_dataset.to_components(
        negative_samples=samples, aux_matrix=test_dataset.interactions
    )
    (
        train_user_x,
        val_user_x,
    ...
    users, items = create_rankings(
        test_dataset, train_dataset, n_samples=100, unravel=True
    )
